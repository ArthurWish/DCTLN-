...
async def get_model(index: int = 0):
    ...
    feature_extractor = nn.DataParallel(total_network.feature_extractor).train(False)
    classifier = nn.DataParallel(total_network.condition_classifier).train(False)
...
async def send_data(data: Item):
    ...
    print('predict_probability: {}'.format(predict_prob))
